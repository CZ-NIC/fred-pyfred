...
Requires: python2 python-omniORB omniORB-servers fred-idl python-clearsilver PyGreSQL < 5.0 python-dns ldns postfix m2crypto
%if 0%{?fedora}
Requires: ldns-utils
%endif
...
%install
PYTHONPATH=%{_topdir}/BUILD/distutils-%{distutils_branch}:$PYTHONPATH python2 setup.py install -cO2 --force --root=$RPM_BUILD_ROOT --record=INSTALLED_FILES --prefix=/usr --install-sysconf=/etc --install-localstate=/var --no-check-deps --drill=/usr/bin/drill --sendmail=/usr/sbin/sendmail
