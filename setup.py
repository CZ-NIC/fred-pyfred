...
#in this list are all modules which are generated by compile_idl,
#that their install will be ensured (via Install.Install.Install.copyIdlStubs method)
g_idlModules=['pyfred.idlstubs', 'pyfred.idlstubs.ccReg', 'pyfred.idlstubs.ccReg__POA']
...
class Install (install.install, object):
    ...
    user_options.append(
            ("omniidl=", "i", "omniidl program used to build stubs [omniidl]"))
    user_options.append(
            ("idldir=",  "d", "directory where IDL files reside [PREFIX/share/idl/fred/]"))
    user_options.append(
            ("idlforce", "o", "force idl stubs to be always generated"))
    ...
    def update_pyfredctl(self, prefix):
        ...
        #search path for pid and fred conf file and replace it with correct one
            ...
            if isDir(os.path.join(src, source)):
                ...
                for i in os.listdir(os.path.join(src, source)):
                    if isFile(os.path.join(src, source, i)):
                        try:
                            shutil.copyfile(os.path.join(src, source, i), os.path.join(dest, source, i))
    ...
    def run(self):
        ...
        self.idlModules=g_idlModules
        self.copyIdlStubs(self.idlModules)
