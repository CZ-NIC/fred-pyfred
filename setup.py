...
# vim: set ts=4 sw=4:
#
import sys, os, string, commands, re
...
class Build (build.build, object):
    ...
    """
    user_options = []
    user_options.append(("prefix=", None, "installation prefix [/usr/local/]"))
    user_options.extend(build.build.user_options)
    user_options.append(("omniidl=", "i", "omniidl program used to build stubs [omniidl]"))
    user_options.append(("idldir=",  "d", "directory where IDL files reside [PREFIX/share/idl/fred/]"))
...
class Install (install.install, object):
    ...
    user_options.append(
            ('dbuser=', None, 'Name of FRED database user [fred]'))
    user_options.append(
            ('dbname=', None, 'Name of FRED database [fred]'))
    user_options.append(
            ('dbhost=', None, 'FRED database host [localhost]'))
    user_options.append(
            ('dbport=', None, 'Port where PostgreSQL database listening [5432]'))
    user_options.append(
            ('dbpass=', None, 'Password to FRED database []'))
    user_options.append(
    ...
    def update_server_config(self, prefix):
        self.find_sendmail()
