...
Lower date and upper date must by both specified or none of them. The format
of DATETIME is '{YYYY}-{MM}-{DD}T{HH}:{MM}:{SS}'.
""")
...
def main():
	...
	for o, a in opts:
		...
		if o in ("-c", "--chunk"):
			...
			u_crdate = a
	#
	if (l_crdate and not u_crdate) or (not l_crdate and u_crdate):
		sys.stderr.write("None of them or both of lower date and upper "
				"date must be specified\n")
		usage()
		sys.exit(1)
	if l_crdate and u_crdate:
		try:
