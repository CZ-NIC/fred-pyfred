...
class UploadTest(unittest.TestCase):
	...
	def test_uploadFromFile(self):
		...
		'''
		cmd = 'filemanager_client --input=%s --mime="text/plain" --type=0' %\
				self.filename
	...
	def test_uploadFromStdin(self):
		...
		'''
		cmd = 'filemanager_client --mime="text/plain" --type=0 <%s' %\
				self.filename
		(status, output) = commands.getstatusoutput(cmd)
		status = os.WEXITSTATUS(status) # translate status
...
class DownloadTest(unittest.TestCase):
	...
	def runTest(self):
		...
		cmd = 'filemanager_client --output="%s" --id=%d' %\
...
class SearchTest(unittest.TestCase):
	...
	def runTest(self):
		...
		Find uploaded file.
		'''
		cmd = 'filemanager_admin_client --chunk=1 --label="%s" '\
